...
impl DerefMut for MachSharedMemory {
    #[inline]
...
impl MachSharedMemory {
    ...
    unsafe fn new_uninitialized(length: usize) -> MachSharedMemory {
        let mut address = 0;
        let result = mach_sys::vm_allocate(mach_task_self(), &mut address, length, 1);
        if result != KERN_SUCCESS {
            panic!("`vm_allocate()` failed: {}", result);
        }
        MachSharedMemory::from_raw_parts(address as *mut u8, length)
    }

    pub fn new(length: usize) -> MachSharedMemory {
        unsafe {
            let mut shared_memory = MachSharedMemory::new_uninitialized(length);
